+++
draft = false
title = 'Apps'
+++

## Open Source

- [Audacity](https://www.audacityteam.org) Audio editing and recording.
- [Cog](https://cog.losno.co) Audio player.
- [CoolTerm](https://freeware.the-meiers.org) Serial port terminal.
- [DBeaver](https://dbeaver.io) Universal Database Tool.
- [Freelens](https://freelensapp.github.io) Kubernetes IDE that provides a graphical interface for managing and monitoring Kubernetes clusters.
- [Gapplin](https://gapplin.wolfrosch.com) SVG viewer.
- [Hex Field](https://hexfiend.com) Hex Editor.
- [HiFidelity](https://github.com/rvarunrathod/HiFidelity) Music player with high-fidelity audio playback.
- [Homebrew](https://brew.sh) Package Manager.
- [Kap](https://getkap.co) Screen recorder.
- [KiCad](https://www.kicad.org) Electronics Design.
- [MacPass](https://macpassapp.org) Password manager.
- [Mole](https://github.com/tw93/Mole) Deep clean and optimize your Mac.
- [NetNewsWire](https://netnewswire.com) RSS reader.
- [Rectangle](https://rectangleapp.com) Move and resize windows.
- [Sniffnet](https://sniffnet.net) Monitor your Internet traffic.
- [The Unarchiver](https://theunarchiver.com) Open any compress archive in seconds.
- [Zed](https://github.com/zed-industries/zed) Code editor.
- [eqMac](https://eqmac.app) Audio Equalizer.
- [iTerm](https://iterm2.com) Terminal emulator.
- [lulu](https://objective-see.org/products/lulu.html) Firewall.

## Comercial

- [Dataflare](https://dataflare.app) Database Manager.
- [Sublime Text](https://www.sublimetext.com) Text Editing.
- [TablePlus](https://tableplus.com) Modern, native, and friendly GUI tool for relational databases: MySQL, PostgreSQL, SQLite & more...
- [Viscosity](https://www.sparklabs.com/viscosity/download) VPN Client.
